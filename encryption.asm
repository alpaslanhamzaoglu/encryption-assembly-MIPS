...
array6: .word 10, 7, 6, 9, 1, 2, 12, 5, 3, 4, 8, 15, 13, 14, 11, 0
array7: .word 9, 2, 15, 8, 0, 12, 3, 6, 4, 13, 1, 14, 7, 11, 10, 5
array8: .word 12, 3, 0, 10, 11, 4, 5, 15, 9, 14, 6, 13, 2, 7, 8, 1
array9: .word 11, 5, 4, 15, 12, 6, 9, 0, 13, 3, 14, 8, 1, 10, 2, 7
array10: .word 10, 7, 6, 9, 1, 2, 12, 5, 3, 4, 8, 15, 13, 14, 11, 0, 9, 2, 15, 8, 0, 12, 3, 6, 4, 13, 1, 14, 7, 11, 10, 5, 12, 3, 0, 10, 11, 4, 5, 15, 9, 14, 6, 13, 2, 7, 8, 1, 11, 5, 4, 15, 12, 6, 9, 0, 13, 3, 14, 8, 1, 10, 2, 7
...
main:
	...
	la $s4, array5
        ...
        #addi $s5, $s5, 0  # i = 0
#Loop:    
    	#beq $s5, 100, Exit # if i != 100
    	#addi $s5, $s5, 1   # i++
    
    	#addi $a1, $zero, 65535 # upper limit 2^16
    	#addi $v0, $zero, 42    # 42 is for random int generation
    	#syscall            
    	#addi $s6, $a0, 0
    
    	#move $a0, $s6
    	#jal nonlinear2    # small table  
    	#move $s6, $v0
...
inverselinear:
	...
	sll $t1, $t0, 10 	 # shift 10
    	srl $t3, $t1, 16	 # upper half
    	andi $t4, $t1, 65535 	 # lower half
    	or $t1, $t3, $t4 	 # left 10 shift
	
    	addi $t2, $t0, 0
    	sll $t2, $t2, 16
    	srl $t2, $t2, 10
    	srl $t3, $t2, 16	 # upper half
    	andi $t4, $t2, 65535 	 # lower half
    	or $t2, $t3, $t4	 # right 10 shift
...
inversew:
	...
	la $t0, keyfour
	lw $t0, 12($t0)
	xor $t0, $t0, $a0 # X xor D
	
	move $a0, $t0
	jal inversef
	move $a0, $v0 # F(X xor D) => 1
	
	la $t0, keyfour
	lw $t0, 8($t0)
	xor $t0, $a0, $t0 # 1 xor C
	
	move $a0, $t0
	jal inversef
	move $a0, $v0 # F(1 xor C) => 2
	
	la $t0, keyfour
	lw $t0, 4($t0)
	xor $t0, $a0, $t0 # 2 xor B
	
	move $a0, $t0
	jal inversef
	move $a0, $v0 # F(2 xor B) => 3
	
	la $t0, keyfour
	lw $t0, 0($t0)
	xor $t0, $a0, $t0 # 3 xor A
